                            ...
                            <div class="col-lg-8 col-xl-8 detail">
                                <div class="product_content_details">
                                    ...
                                    <div class="product_ratings">
                                        ...
                                        <span><a href="#customer_reviews">{{getNumberTranslate(sprintf("%.2f",$rating))}}/{{getNumberTranslate(5)}} ({{($total_review<10 && $total_review>0)?'0':''}}{{getNumberTranslate($total_review)}} {{__('defaultTheme.review')}})</a></span>
                                    </div>
                            ...
                            <div class="product_details_dec mb_76">
                                ...
                                <div class="product_details_dec_body">
                                    <div class="single_desc style2 mb_20">
                                        <p class="f_w_500 m-0"><b>{{ __('common.brand') }}</b>: {{@$product->product->brand->name}}</p>
                                        <p class="f_w_500 m-0"><b>{{ __('common.model_number') }}</b>: {{@$product->product->model_number}}</p>
                                        <p class="f_w_500 m-0"><b>{{ __('common.availability') }}</b>:
                                            @if ($product->stock_manage == 1 && $product->skus->first()->product_stock >= $product->product->minimum_order_qty)
                                                {{ __('common.in_stock') }}
                                            @elseif($product->stock_manage == 0)
                                                {{ __('common.in_stock') }}
                                        ...
                                        <p class="f_w_500 m-0"><b>{{ __('common.product') }} {{ __('product.sku') }}</b>:
                                            @foreach ($product->product->skus as $sku)
                                                {{$sku->sku}} &nbsp; &nbsp;
                                            @endforeach
                                        </p>
                                        <p class="f_w_500 m-0"><b>{{ __('common.minimum_order_quantity') }}</b>: {{getNumberTranslate(@$product->product->minimum_order_qty)}}</p>
                                        <p class="f_w_500 m-0"><b>{{ __('common.maximum_order_quantity') }}</b>: {{getNumberTranslate(@$product->product->max_order_qty)}}</p>
                                        <p class="f_w_500 m-0"><b>{{ __('common.listed_date') }}</b>: {{date(app('general_setting')->dateFormat->format, strtotime(@$product->product->created_at))}}</p>
                                    </div>
                                        ...
                                        <div class="col-xl-4 col-lg-4 col-md-6">
                                            <div class="product_widget5 mb_30">
                                                ...
                                                <div class="product__meta">
                                                    ...
                                                    <span class="product_banding ">{{ @$recent_viewed_product->brand->name ?? __('generic') }}</span>
