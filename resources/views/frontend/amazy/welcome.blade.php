...
@section('content')
    ...
    <x-top-brand-component class="bg-light"/>
<!-- home_banner::end  -->
@php
    $best_deal = $widgets->where('section_name','best_deals')->first();
@endphp
<div id="best_deals" style="width: 80%; margin-left: 50%; transform:translateX(-50%); background: whitesmoke; border-radius: 5px;" class="amaz_section section_spacing {{$best_deal->status == 0?'d-none':''}} mb-4">
    <div class="container">
        <div class="row">
            <div class="col-12">
                    ...
                    @foreach($best_deal->getProductByQuery() as $key => $product)
                        <div class="product_widget5 mb_30 style2 deals-product" style="margin-right: 15px; padding-top: 0!important;">
                            ...
                            <div class="product__meta">
                                ...
                                <span class="product_banding ">{{ $product->brand->name ?? __('generic') }}</span>
                                <div class="stars">
                                    @php
...
@endphp
<div id="feature_categories" style="width: 80%; margin-left: 50%; transform:translateX(-50%); background: whitesmoke; border-radius: 5px;" class="amaz_section {{$feature_categories->status == 0?'d-none':''}} mb-4">
    <div class="container">
        <div class="row">
            <div class="col-12">
...
<div id="filter_category" class="amaz_section section_spacing2 {{$filter_category->status == 0?'d-none':''}} pt-0">
    <div class="container">
        @if($category)
                        ...
                        <div class="amaz_fieature_active fieature_crousel_area owl-carousel">
                            @foreach($top_rating->getHomePageProductByQuery() as $key => $product)
                                <div class="product_widget5 style2 mb_30">
                                    ...
                                    <div class="product__meta">
                                        <a href="{{singleProductURL($product->seller->slug, $product->slug)}}">
                                            <h4 class="text-nowrap" style="font-size: 13px;">@if ($product->product_name) {{ textLimit(@$product->product_name, 35) }} @else {{ textLimit(@$product->product->product_name, 35) }} @endif</h4>
                                        </a>
                                        <span class="product_banding ">{{ $product->brand->name ?? __('generic') }}</span>
                                        <div class="stars">
                                            @php
                            ...
                            <div class="product_widget5 style2 mb_30">
                                ...
                                <div class="product__meta">
                                    ...
                                    <div class="product_prise">
                                        <p>
                                            @if($product->hasDeal)
                                                {{single_price(selling_price($product->skus->first()->selling_price,$product->hasDeal->discount_type,$product->hasDeal->discount))}}<br>
                                            @else
                                                @if($product->hasDiscount == 'yes')
                                                    {{single_price(selling_price(@$product->skus->first()->selling_price,@$product->discount_type,@$product->discount))}}<br>
                
                                                @else
                                                    {{single_price(@$product->skus->first()->selling_price)}}<br>
                                                @endif
                                            @endif
                                        </p>
                                        <a class="add_cart add_to_cart addToCartFromThumnail" data-producttype="{{ @$product->product->product_type }}" data-seller={{ $product->user_id }} data-product-sku={{ @$product->skus->first()->id }} 
...
@endphp
<div id="discount_banner" class="amaz_section amaz_deal_area {{$discount_banner->status == 0?'d-none':''}} mb-4 d-flex" style="width: 80%; margin-left: 50%; transform:translateX(-50%);">
    <div class="container pt-3 d-flex" style="background: whitesmoke; border-radius: 5px; flex-direction:row; flex-wrap:nowrap; overflow-y: hidden; overflow-x: scroll;"><div class="banner-scrollers"><button id="left"><i class="ti-angle-left"></i></button><button id="right"><i class="ti-angle-right"></i></button></div>
        <div class="col-xl-4 col-md-6 col-sm-12 col-lg-4 mb_20 single_ad {{!@$discount_banner->customSection->field_4?'d-none':''}}">
        ...
        <div class="col-xl-4 col-md-6 col-lg-4 col-sm-12 mb_20 single_ad {{!@$discount_banner->customSection->field_5?'d-none':''}}">
        ...
        <div class="col-xl-4 col-md-6 col-lg-4 col-sm-12 mb_20 single_ad {{!@$discount_banner->customSection->field_6?'d-none':''}}">
...
@endphp
<div class="amaz_recomanded_area {{$more_products->status == 0?'d-none':''}} mb-4">
