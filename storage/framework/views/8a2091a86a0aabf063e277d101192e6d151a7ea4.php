<?php $__env->startPush('scripts'); ?>
    ...
    $(document).ready(function(){
        ...
        // var product_unit = $('#unit_type_id').val();
        var tags = $('#tags').val();
        var selling_price = $('#selling_price').val();
            //first slide
            $('#sectionNext2').on('click', function(e){
                if(($('#product_name').val().length == 0) || ($('#category_id').val().length == 0) || ($('#tags').val().length == 0)){
                    e.preventDefault();
                    $('.filling_message').html('Fill the required inputs').show();
                    if($('#product_name').val().length == 0){
                        $('#error_product_name').text('please input product name');
                    }
                    if($('#category_id').val().length == 0){
                        $('#error_category_ids').text("please select category");
                    }
                    if($('#tags').val().length == 0){
                        $('#error_tags').text("please input tags");
                    }
                }
                else if($('#minimum_order_qty').val() == '0' || $('#minimum_order_qty').val() == ''){
                    e.preventDefault();
                    alert('Minimum quantity cannot be zero or empty!');
                    if($('#minimum_order_qty').val() == '0'){
                        $('#minimum_order_qty').val("0");
                    }
                    ...
                    $('.filling_message').hide();
                    $('#error_product_name').text('');
                    $('#error_category_ids').text('');
                    $('#error_tags').text('');

                    //taking product name value to SEO title
                    $('.meta_title').val($('#product_name').val());
                }
            });
            //second slide
            $('#sectionNext3').on('click', function(e){
                if($('#selling_price').val() == 0 || $('#selling_price').val() == '' || $("#discount").val() == ''){
                    e.preventDefault();
                    $('.filling_message2').html('Fill the required inputs').show();

                    if($('#selling_price').val() == 0 || $('#selling_price').val() == ''){
                        $('#error_selling_price').text("please input selling price");
                    }
                    if($("#discount").val() == ''){
                        $('#error_discunt').text("please input discount minimum is 0");
                    }
                }
                else{
                    $('.filling_message2').hide();
                    $('#error_selling_price').text('');
                    $('#error_discunt').text('');
                }
            });

            //taking description content to meta description
            $(document).on("blur", ".description-part2 .note-editable", function(){
                var text = $(".description-part2 .note-editable")[0].innerText;
                function truncate(str, no_words){
                    return str.split(' ').splice(0, no_words).join(' ');
                }
                $('.meta_description').text(truncate(text, 25));
            });
            $(document).on("blur", ".description-part1 .note-editable", function(){
                var text = $(".description-part1 .note-editable")[0].innerText;
                function truncate(str, no_words){
                    return str.split(' ').splice(0, no_words).join(' ');
                }
                $('.meta_description').text(truncate(text, 25));
            ...
            //limiting the number product images
            $(document).on("click", '#all_files_div .single_files', function(e){
                var selected = $('#all_files_div .single_files[data-selected="true"]').length;
                if(selected > 8){
                    $('.next_prev_btn_div button').attr('disabled', 'disabled');
                    alert('Maximum number of images accepted is 8.');
                }
                else{
                    $('.next_prev_btn_div button').removeAttr('disabled');
                    $('.next_prev_btn_div button').click(function(){
                        $('#meta_image_input').val($('#product_images_array').val().split(',')[0]);
                    })
                }
            });//product_images_array, meta_image_input
            
            //taking the first image of product images and make it meta image
            ...
            $('.tip-button2').on('click', function(e){
                e.preventDefault();
                $('.tip2').toggle();
            });
    });
    ...
    (function($) {
    	"use strict";
        var gold_module_check = "<?php echo e(isModuleActive('GoldPrice')); ?>";
        $(document).ready(function() {

            if("<?php echo e($errors->has('sku.*')); ?>"){
            ...
            $(document).on('click','.saveBtn',function() {
                ...
                $('#error_product_name_<?php echo e(auth()->user()->lang_code); ?>').text('');
                <?php else: ?>
                $('#error_product_name').text('');
                <?php endif; ?>
                $('#error_category_ids').text('');
                ...
                <?php if(isModuleActive('FrontendMultiLang')): ?>
                    if ($("#product_name_<?php echo e(auth()->user()->lang_code); ?>").val() === '') {
                        requireMatch = 1;
                        $('#error_product_name_<?php echo e(auth()->user()->lang_code); ?>').text("<?php echo e(__('product.please_input_product_name')); ?>");
                    }
                <?php else: ?>
                    if ($("#product_name").val() === '') {
                        requireMatch = 1;
                        $('#error_product_name').text("<?php echo e(__('product.please_input_product_name')); ?>");
                    }
                <?php endif; ?>

                if ($("#category_id").val().length < 1) {
                    requireMatch = 1;
                    $('#error_category_ids').text("<?php echo e(__('product.please_select_category')); ?>");

                }
                if ($("#minimum_order_qty").val() === '') {
                    requireMatch = 1;
                    $('#error_minumum_qty').text("<?php echo e(__('product.please_input_minimum_order_qty')); ?>");

                }
                if ($("#selling_price").val() === '') {
                    requireMatch = 1;
                    $('#error_selling_price').text("<?php echo e(__('product.please_input_selling_price')); ?>");

                }
                // if ($("#tax").val() === '') {
                //     requireMatch = 1;
                //     $('#error_tax').text("<?php echo e(__('product.please_input_tax')); ?>");

                // }
                if ($("#discount").val() === '') {
                    requireMatch = 1;
                    $('#error_discunt').text("<?php echo e(__('product.please_input_discount_minimum_0')); ?>");

                }
                    if ($("#tags").val() === '') {
                        requireMatch = 1;
                        $('#error_tags').text("<?php echo e(__('product.please_input_tags')); ?>");

                    }
                if ($('input[name=product_type]:checked').val() === '2' && $(".choice_attribute").val().length === 0) {
                    requireMatch = 1;
                    toastr.warning("<?php echo e(__('product.please_select_attribute')); ?>");
            ...
            $(document).on('change', '#choice_attributes', function() {
                ...
                if(exsist > 0){
                    toastr.error("<?php echo e(__('marketing.this_item_already_added_to_list')); ?>");
            ...
            function getAttributeData(a_id){
                $.post('<?php echo e(route('product.attribute.values')); ?>', {
            ...
            $(document).on('change', '#tax_type', function(event){
                ...
                $('#pre-loader').removeClass('d-none');
                $.post("<?php echo e(route('product.change-gst-group')); ?>", data, function(response){
            ...
            <?php if(isModuleActive('FrontendMultiLang')): ?>
                $(document).on('keyup', '#category_name<?php echo e(auth()->user()->lang_code); ?>', function(event){
                    processSlug($('#category_name<?php echo e(auth()->user()->lang_code); ?>').val(), '#category_slug');
                });
            <?php else: ?>
            ...
            $(document).on('submit', '#add_category_form',  function(event) {
                ...
                if(single_image ==1){
                    ...
                    if (size > 1) {
                       toastr.error("<?php echo e(__('product.file_must_be_less_than_1_mb')); ?>","<?php echo e(__('common.error')); ?>");
                       return false;
                    }
                    var value=$('#image').val();
                    ...
                    if ($.inArray(type, validFileExtensions) == -1) {
                       ...
                       toastr.error("<?php echo e(__('product.invalid_type_type_should_be_jpeg_jpg_png')); ?>","<?php echo e(__('common.error')); ?>");
                       return false;
                    }
                ...
                $.ajax({
                    url: "<?php echo e(route('product.category.store')); ?>",
                    type: "POST",
                    cache: false,
                    contentType: false,
                    processData: false,
                    ...
                    success: function(response) {
                        ...
                        dynamicSelect2WithAjax(".category_id", "<?php echo e(url('/products/get-category-data')); ?>", "GET");
                        dynamicSelect2WithAjax("#parent_id", "<?php echo e(url('/products/get-category-data')); ?>", "GET");
                        ...
                        `
                            <label class="primary_input_label" for=""><?php echo e(__('common.upload_photo')); ?> (<?php echo e(__('common.file_less_than_1MB')); ?>)</label>

                            <div class="primary_input mb-25">
                                <div class="primary_file_uploader">
                                  <input class="primary-input" type="text" id="image_file" placeholder="<?php echo e(__('common.browse_image_file')); ?>" readonly="">
                                  <button class="" type="button">
                                      <label class="primary-btn small fix-gr-bg" for="image"><?php echo e(__("common.browse")); ?> </label>
            ...
            $(document).on('submit', '#create_brand_form', function(event){
                ...
                $.ajax({
                    url: "<?php echo e(route('product.brand.store')); ?>",
                    type:"POST",
                    cache: false,
                    contentType: false,
                    processData: false,
                    ...
                    success:function(response){
                        ...
                        toastr.success('<?php echo e(__("product.brand")); ?> <?php echo e(__("common.created_successfully")); ?>');
                        $('#pre-loader').addClass('d-none');
                        $('#create_brand_modal').modal('hide');
                        dynamicSelect2WithAjax(".brand_id", "<?php echo e(route('product.brands.get-by-ajax')); ?>", "GET");
            ...
            $(document).on('submit', '#create_attribute_form', function(event){
                ...
                $.ajax({
                    url: "<?php echo e(route('product.attribute.store')); ?>",
                    type:"POST",
                    cache: false,
                    contentType: false,
                    processData: false,
                    ...
                    success:function(response){
                        ...
                        toastr.success('<?php echo e(__("product.attribute")); ?> <?php echo e(__("common.created_successfully")); ?>');
                        $('#pre-loader').addClass('d-none');
            ...
            $(document).on('change', '.prod_type', function(){
                var checkWholeSaleM = '<?php echo e((isModuleActive('WholeSale')? 1: null)); ?>';
        ...
        function get_combinations(old = false) {
            ...
            if(old){
                formdata.push({name: 'old_sku_price', value: <?php echo json_encode(old('selling_price_sku', []), 512) ?>});
                formdata.push({name: 'old_sku_stock', value: <?php echo json_encode(old('sku_stock', []), 512) ?>});
                formdata.push({name: 'old_sku', value: <?php echo json_encode(old('sku', []), 512) ?>});
            }
            $.ajax({
                type: "POST",
        ...
        <?php if(isModuleActive('FrontendMultiLang')): ?>
            $(document).on('focusout', '#product_name_<?php echo e(auth()->user()->lang_code); ?>', function(){
                // tag get
                $("#tag_show").html('<li></li>');
        ...
        $(document).on('keyup', '#rsearch_products', function(event){
            event.preventDefault();
                var search = $(this).val();
                $.ajax({
                    url: "<?php echo e(route('product.related.product')); ?>",
                    type: "GET",
                    data: {
                        "search": search,
                        "spage": 'create',
        ...
        $(document).on('keyup', '#upsale_search_products', function(event){
            event.preventDefault();
                var search = $(this).val();
                $.ajax({
                    url: "<?php echo e(route('product.upsale.product')); ?>",
                    type: "GET",
                    data: {
                        "search": search,
                        "spage": 'create',
        ...
        $(document).on('keyup', '#crosssale_search_products', function(event){
            event.preventDefault();
                var search = $(this).val();
                $.ajax({
                    url: "<?php echo e(route('product.crosssale.product')); ?>",
                    type: "GET",
                    data: {
                        "search": search,
                        "spage": 'create',
...
<?php $__env->stopPush(); ?>
<?php /**PATH C:\xampp\htdocs\pomalo\Modules/Product\Resources/views/products/create_script.blade.php ENDPATH**/ ?>