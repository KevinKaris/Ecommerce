...
<script>
    ...
    $(document).ready(function(){
            var banner_img_size = $('#discount_banner .col-md-6').outerWidth();
        $('.banner-scrollers #left').on('click', function(){

            $('#discount_banner > .container').animate({scrollLeft: '-='+banner_img_size}, 500);
        });
        $('.banner-scrollers #right').on('click', function(){
...
<script src="<?php echo e(asset(asset_path('js/custom.js'))); ?>"></script>
...
<script>
    (function($){
        $(document).ready(function(){
            ...
            }, <?php echo e($popupContent->second); ?>*1000);

            $(document).on('click', '#promotion_close', function(event){
                event.preventDefault();
                $.get('<?php echo e(url("/frontend/close-promotion")); ?>',function(data){
            ...
            $(document).on('submit','#subscriptionForm', function(event) {
                ...
                $('#subscribeBtn').text('<?php echo e(__("common.submitting")); ?>');
                ...
                $.ajax({
                    url: "<?php echo e(route('subscription.store')); ?>",
                    type: "POST",
                    cache: false,
                    contentType: false,
                    processData: false,
                    ...
                    success: function(response) {
                        ...
                        if(response.msg == 'success'){
                            $('.message_div').html(`
                                <span class="text-success"><?php echo e(__('defaultTheme.subscribe_successfully')); ?></span>
                            `);
                        }
                        else if(response.msg == 'verify_link_send'){
                            $('.message_div').html(`
                                <span class="text-success"><?php echo e(__('amazy.Verify link send to on you email.')); ?></span>
                            `);
                        }
                        $("#subscribeBtn").prop('disabled', false);
                        $('#subscribeBtn').text("<?php echo e(__('defaultTheme.subscribe')); ?>");
            ...
            $(document).on('submit','#modalSubscriptionForm', function(event) {
                ...
                $('#modalSubscribeBtn').text('<?php echo e(__("common.submitting")); ?>');
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if(response.msg == 'success'){
                            toastr.success("<?php echo e(__('defaultTheme.subscribe_successfully')); ?>", "<?php echo e(__('common.success')); ?>");
                        }
                        else if(response.msg == 'verify_link_send'){
                            toastr.success("<?php echo e(__('amazy.Verify link send to on you email.')); ?>", "<?php echo e(__('common.success')); ?>");
                        }
                        $("#modalSubscribeBtn").prop('disabled', false);
                        $('#modalSubscribeBtn').text("<?php echo e(__('defaultTheme.subscribe')); ?>");
            ...
            $(document).on('blur', '#subscription_email_id', function(event){
                $(this).attr('placeholder','<?php echo e(__("defaultTheme.enter_email_address")); ?>');
            });
            ...
            function infinteLoadMore(page, new_url, tbl_name) {
                ...
                .done(function (response) {
                    if (response.length == 0) {
                        ...
                        toastr.warning("<?php echo e(__('defaultTheme.no_more_data_to_show')); ?>");
                        return;
                    }
                    $('#pre-loader').hide();
            ...
            $(document).on('submit','#search_form', function(event){
                ...
                var genUrl = "<?php echo e(url('/category')); ?>"+'/'+input_data+'?item=search&category='+$('.category_id option:selected').val();
                            ...
                            productData += `
                                <a data-type="product" href="${item.url}" class="store_link product_search_single d-flex align-items-center mb_10 gap_10">
                                    <div class="product_info d-flex align-items-center flex-fill gap_10">
                                        <div class="thumb">
                                                <img data-src="${item.thumb_img}" alt="" src="<?php echo e(showImage(themeDefaultImg())); ?>" class="lazyload">
                                        </div>
                                ...
                                sellerData += `
                                    <a href="${url}" data-type="product" class="store_link product_search_single d-flex align-items-center gap_10">
                                        <div class="product_info flex-fill d-flex gap_10 mb_10 align-items-center">
                                            <div class="thumb">
                                                <img data-src="${item.avater}" alt="" src="<?php echo e(showImage(themeDefaultImg())); ?>" class=lazyload>
                                            </div>
                                            <div class="product_info_text">
                    ...
                    if(val == '+'){
                        if(max_qty != ''){
                            if (parseInt(qty) < parseInt(product_stock)) {
                                ...
                                toastr.warning("<?php echo e(__('defaultTheme.no_more_stock_available')); ?>","<?php echo e(__('common.warning')); ?>");
                            }
                        }
                        else{
                            if (parseInt(qty) < parseInt(product_stock)) {
...
<?php echo $__env->make('frontend.amazy.partials.global_script', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\pomalo\resources\views/frontend/amazy/partials/_script.blade.php ENDPATH**/ ?>